...
histories = {
  ajax_back: function(value){
//     value != 'undefined';
//     alert(links_array);
    if(links_array.length > 1){
      for(var i = 0; i < links_array.length; i++){
        if(links_array[i] == value){
          if(i>0)
            var url = links_array[i-1];
          a = document.createElement("a");
          a.href="#";
          a.innerHTML = "back";
          a.onclick = function() {links_array.pop();cookies.set('active_page', url); Element.show('spinner');new Ajax.Request(url, {asynchronous:true, evalScripts:true, method:'get', onComplete:function(request){Element.hide('spinner'); }})};
        }
      }
      $('back').appendChild(a);
    }
...
var Filter = {
  set_cookies: function(object, filter){
  ...
  get_url: function(){
    ...
    for(var i = 0; i < docs.length; i++)
      if(docs[i].checked == true && (docs[i].id.match('states_')=='states_' || docs[i].id.match('users_')=='users_'))
        url += docs[i].name +'=' + docs[i].value + '&';
//     alert(url);
    var main_url = document.location.href
    main_url.split('/')
    if (url.length > 0 )
      url = document.location.href.split('#')[0] + '/tickets/?' + url;
    else
      url = cookies.get('active_page');
    return url;
  },
  set_url: function(){
    ...
    a.href="#";
    a.innerHTML = "My last filter";
//     alert(Filter.get_url());
    a.onclick = function() {links_array.pop(); cookies.set('active_page', url); Element.show('spinner');new Ajax.Request(url, {asynchronous:true, evalScripts:true, method:'get', onComplete:function(request){Element.hide('spinner'); }})};
    cookies.set('active_page', url);
    li.appendChild(a)
    new Insertion.Top($('tickets'), li);
  }
};
...
//         link == value;
//       }
      if(links_array[len - 1] == value)
        links_array[len - 1] = value;
      else
        links_array[len] = value;
     else
       links_array[0] = value;
  },
...
var CurPage= {
  current_page: function(project){
//     alert(url.match('user/'))
    var url = cookies.get('active_page');
//     if(url != 'undefined'){
      links_array[0] = url;
      if(url.match(project) == project || url.match('user/') == 'user/')
      {
        $('main_page').hide();
        Element.show('spinner');
        new Ajax.Request(url, {asynchronous:true, evalScripts:true, method:'get', onComplete:function(request){Element.hide('spinner'); $('main_page').show();}});
      }
//     }
//     links_array[0] = '/project/bugtrack';
  }
};

var Popup={
  popup_ticket: function(url, id) {
    if(Element.visible($('ticket_'+ id))){
      $('ticket_'+ id).hide();
      $('menu_ticket_'+ id).style.background ="white";
    }
    else{
      var elems = document.getElementsByClassName('ticket');
      for (var i = 0; i < elems.length; i += 1){
          $('menu_' + elems[i].id).style.background = "white";
//           $(elems[i].id).clonePosition('menu_' + elems[i].id)
          $(elems[i].id).hide();
      }
      Element.show('spinner');
      $('menu_ticket_'+ id).style.background ="#e0e0e0";
